...
expName = 'Abfrage_NSWP'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/charlotte/Desktop/DOCUMENTS/UNI/MEDIZIN/PROMOTION/PsychoPy/MRT-Pilot/NSWP_Pilot_v2/Abfrage_NSWP_Pilot_v2_lastrun.py',
...
Anleitungstext1 = visual.TextStim(win=win, name='Anleitungstext1',
    text='Im Folgenden wird Ihnen auf der linken Seite des Bildschirms ein Wort gezeigt.\n\nAuf der rechten Seite werden vier Antwortmöglichkeiten zu sehen sein. \n\nBitte antworten Sie immer erst, wenn die Pfeile zwischen den Antwortmöglichkeiten auftauchen.\nSie haben dann 4 Sekunden um mit einem Mausklick das korrekte Wortpaar aus der Lernphase zu vervollständigen.\n\n-> Weiter mit einem Mausklick',
    font='Arial',
...
if len(mouseNext.x): thisExp.addData('mouseNext.x', mouseNext.x[0])
if len(mouseNext.y): thisExp.addData('mouseNext.y', mouseNext.y[0])
if len(mouseNext.leftButton): thisExp.addData('mouseNext.leftButton', mouseNext.leftButton[0])
if len(mouseNext.midButton): thisExp.addData('mouseNext.midButton', mouseNext.midButton[0])
if len(mouseNext.rightButton): thisExp.addData('mouseNext.rightButton', mouseNext.rightButton[0])
if len(mouseNext.time): thisExp.addData('mouseNext.time', mouseNext.time[0])
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('NSWP_Abfrage_Stimulus_List.xlsx', selection='0:60'),
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    AbfrageComponents = [bottomRight, topRight, bottomLeft, topLeft, Pfeile, mouse_resp, Stim_Background, Stimulus, topLeftTxt, bottomLeftTxt, topRightTxt, bottomRightTxt]
        ...
        if mouse_resp.status == STARTED:  # only update if started and not finished!
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    gotValidClick = False
                    for obj in [topRight, topLeft, bottomRight, bottomLeft]:
    ...
    if len(mouse_resp.x): trials.addData('mouse_resp.x', mouse_resp.x[0])
    if len(mouse_resp.y): trials.addData('mouse_resp.y', mouse_resp.y[0])
    if len(mouse_resp.leftButton): trials.addData('mouse_resp.leftButton', mouse_resp.leftButton[0])
    if len(mouse_resp.midButton): trials.addData('mouse_resp.midButton', mouse_resp.midButton[0])
    if len(mouse_resp.rightButton): trials.addData('mouse_resp.rightButton', mouse_resp.rightButton[0])
    if len(mouse_resp.time): trials.addData('mouse_resp.time', mouse_resp.time[0])
    if len(mouse_resp.clicked_name): trials.addData('mouse_resp.clicked_name', mouse_resp.clicked_name[0])
