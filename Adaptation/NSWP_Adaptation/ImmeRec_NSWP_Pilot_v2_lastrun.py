...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='H:\\Luebeck\\Projects\\Juniorfoerderung\\Tasks\\Pilot\\NSWP\\ImmeRec_NSWP_Pilot_v2_lastrun.py',
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('NSWP_ImmRec_Stimulus_List.xlsx', selection='0:60'),
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if mouse_resp.status == STARTED:  # only update if started and not finished!
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    gotValidClick = False
                    for obj in [topLeft, bottomLeft, topRight, bottomRight]:
        ...
        if Stim_Background.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Stim_Background.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
