...
expName = 'ImmediateRecall_FPA'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/charlotte/Desktop/DOCUMENTS/UNI/MEDIZIN/PROMOTION/Behaviour/PsychoPy/Measurement/FPA_Measurement/ImmeRec_FPA_Pilot_v1.py',
...
Anleitungstext1 = visual.TextStim(win=win, name='Anleitungstext1',
    text='Im Folgenden wird Ihnen für sieben Sekunden auf der linken Seite des Bildschirms eine Figur gezeigt.\n\nAuf der rechten Seite werden vier Antwortmöglichkeiten zu sehen sein. \n\n-> Weiter mit einem Mausklick',
    font='Arial',
...
Anleitungstext2 = visual.TextStim(win=win, name='Anleitungstext2',
    text='Bitte antworten Sie immer erst, wenn die Pfeile zwischen den Antwortmöglichkeiten auftauchen.\nSie haben dann noch 4 Sekunden um mit einem Mausklick das korrekte Figurenpaar aus der Lernphase zu vervollständigen.\n\nNach Ihrer Antwort wird Ihnen das richtige Figurenpaar angezeigt.\n\nZwischen den Aufgaben wird es kurze Pausen geben.\n\n-> Weiter mit einem Mausclick',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('FPA_ImmRec_Stimulus_List.xlsx', selection='0:60'),
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    Figure1.setImage(Cue)
    Figure2.setImage(Target)
    # keep track of which components have finished
    CorrectComponents = [Figure1, Figure2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *Figure1* updates
        if Figure1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Figure1.frameNStart = frameN  # exact frame index
            Figure1.tStart = t  # local t and not account for scr refresh
            Figure1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Figure1, 'tStartRefresh')  # time at next scr refresh
            Figure1.setAutoDraw(True)
        if Figure1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Figure1.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                Figure1.tStop = t  # not accounting for scr refresh
                Figure1.frameNStop = frameN  # exact frame index
                win.timeOnFlip(Figure1, 'tStopRefresh')  # time at next scr refresh
                Figure1.setAutoDraw(False)
        
        # *Figure2* updates
        if Figure2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Figure2.frameNStart = frameN  # exact frame index
            Figure2.tStart = t  # local t and not account for scr refresh
            Figure2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Figure2, 'tStartRefresh')  # time at next scr refresh
            Figure2.setAutoDraw(True)
        if Figure2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Figure2.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                Figure2.tStop = t  # not accounting for scr refresh
                Figure2.frameNStop = frameN  # exact frame index
                win.timeOnFlip(Figure2, 'tStopRefresh')  # time at next scr refresh
                Figure2.setAutoDraw(False)
    ...
    for thisComponent in CorrectComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('Figure1.started', Figure1.tStartRefresh)
    trials.addData('Figure1.stopped', Figure1.tStopRefresh)
    trials.addData('Figure2.started', Figure2.tStartRefresh)
    trials.addData('Figure2.stopped', Figure2.tStopRefresh)
